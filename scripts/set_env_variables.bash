...
project_name='ReinforcedGameLearning'
project_short='rgl'
if [ -n "$PROJECT_SHORT" ]
then
    if [ "$PROJECT_SHORT" != "$project_short" ]
    then
        set +x
...
if [ -z $DATADIR ]
then
    export DATADIR=/mapr/738.mbc.de/data/training/workspace/rd/athena/atplid/$USER/data
fi
if [ -z $EXTERNAL_DATADIR ]
then
    export EXTERNAL_DATADIR="$DATADIR/ext"
fi
if [ -z $OUTPUT_DATADIR ]
then
    export OUTPUT_DATADIR="$DATADIR/$PROJECT_SHORT/out"
else
    if [ "/out" != ${OUTPUT_DATADIR: -4} ]
    then
...
echo "EXTERNAL_DATADIR:  "$EXTERNAL_DATADIR
